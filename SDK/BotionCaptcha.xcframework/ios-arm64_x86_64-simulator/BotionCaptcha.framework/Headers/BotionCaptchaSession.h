...
@class BotionCaptchaSessionConfiguration, BOCError;

@protocol BotionCaptchaSessionTaskDelegate;

/// 验证会话(Captcha session)
@interface BotionCaptchaSession : NSObject

/// 验证会话任务的代理(Captcha session task delegate)
@property (nullable, nonatomic, weak) id<BotionCaptchaSessionTaskDelegate> delegate;
...
@property (nonnull, readonly, nonatomic, strong) BotionCaptchaSessionConfiguration *configuration;
...
+ (instancetype)sessionWithCaptchaID:(NSString *)captchaID
                       configuration:(nullable BotionCaptchaSessionConfiguration *)configuration;
...
@protocol BotionCaptchaSessionTaskDelegate <NSObject>
...
- (void)botionCaptchaSession:(BotionCaptchaSession *)captchaSession
             didReceiveError:(BOCError *)error;
